...
if __name__ == '__main__':
    ...
    load_ae(directory)
    actions_invalid = generate_random_action(actions_valid, ae)
    print("invalid7",actions_invalid.shape, "--- sucessors are random reconstructable states")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
    print("type2 error: ",np.sum(np.round(discriminator.discriminate(actions_invalid,batch_size=1000))))
    ...
    suc_invalid = np.copy(suc)
    random.shuffle(suc_invalid)
    actions_invalid = np.concatenate((pre,suc_invalid),axis=1)
    actions_invalid = set_difference(actions_invalid, actions_valid)
    print("invalid2",actions_invalid.shape, "--- generated by swapping successors; pre/suc are correct states")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
    print("type2 error: ",np.sum(np.round(discriminator.discriminate(actions_invalid,batch_size=1000))))
    ...
    suc2 = np.copy(pre2)
    random.shuffle(suc2)
    actions_invalid = np.concatenate((pre2,suc2),axis=1)
    actions_invalid = set_difference(actions_invalid, actions_valid)
    print("invalid5",actions_invalid.shape, "--- generated by shuffling all valid states; pre/suc are correct but possibly unknown states")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
    print("type2 error: ",np.sum(np.round(discriminator.discriminate(actions_invalid,batch_size=1000))))
    ...
    actions_invalid = np.concatenate((pre2, pre2), axis=1)
    print("invalid6",actions_invalid.shape, "--- invalid actions generated from nop")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
    print("type2 error: ",np.sum(np.round(discriminator.discriminate(actions_invalid,batch_size=1000))))

    actions_invalid = actions_valid.copy()
    actions_invalid[:,:N] = np.random.randint(0,2,(len(actions_valid),N),dtype=np.int8)
    print("invalid3",actions_invalid.shape, "--- pre are generated by random bits, suc are correct states")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
    
    actions_invalid = actions_valid.copy()
    actions_invalid[:,N:] = np.random.randint(0,2,(len(actions_valid),N),dtype=np.int8)
    print("invalid4",actions_invalid.shape, "--- suc are generated by random bits, pre are correct states")
    discriminator.report(actions_invalid,train_data_to=np.zeros((len(actions_invalid),)))
