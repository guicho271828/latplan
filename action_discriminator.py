...
def prepare_oae_validated(known_transisitons):
    ...
    return (default_networks['Discriminator'], *prepare_binary_classification_data(y_positive, y_negative))
...
def load():
    global discriminator
    try:
        discriminator = default_networks['PUDiscriminator'](sae.local("_ad/")).load()
    except:
        discriminator = default_networks['Discriminator'](sae.local("_ad/")).load()
...
def main(directory, mode="test", input_type="prepare_oae_PU3"):
    ...
    oae = default_networks['ActionAE'](sae.local("_aae/")).load()
    cae = default_networks['SimpleCAE'](sae.local("_cae/")).load(allow_failure=True)
    sd3 = default_networks['PUDiscriminator'](sae.local("_sd3/")).load()
