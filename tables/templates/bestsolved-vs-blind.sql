...
with
    ...
    select heuristics, generator, comment, aeclass, time_start, found, max(valid), optimal, exhausted
      from (
        select heuristics,
          ...
          from planning
         where aeclass in (select aeclass from AECLASS)
           and noise = NOISE
           and cycle = CYCLE
  ...
  x as (
    select *
      from best_stat
     where heuristics = 'blind'),
  y as (
    select *
      from best_stat
     where heuristics in ('lmcut', 'mands', 'lama'))
select GROUP.id,
  ...
  from x
         ...
         join heuristics
             on y.heuristics = heuristics.key
         join heuristics2
             on y.heuristics = heuristics2.key
