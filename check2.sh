...
# check2 shows the results per optimal length
...
fn2(){
    ...
    fn "map(select(.problem)) | group_by([(.problem[0:3] | tonumber),$keys1]) | map([ (.[0].problem[0:3] | tonumber), $keys2, (map(select(.valid and ((.problem[0:3] | tonumber)==.statistics.cost))) | length), (map(select(.valid)) | length), (map(select(.found)) | (map(.plan_count)|add)//length), length ] | join(\" \"))" \
        | jq -r '.[]' | column -t
}

fn2 .domain "(.domainfile[-2]+.aae+.heuristics)"
